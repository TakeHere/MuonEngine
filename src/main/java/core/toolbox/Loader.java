...
import core.objects.models.TextureData;
...
public class Loader {
    ...
    public RawModel loadToVAO(float[] positions, int dimensions){
        ...
        return new RawModel(vaoID, positions.length / dimensions, positionsVector.toArray(new Vector3f[positionsVector.size()]));
    }
    ...
    public int loadCubeMap(String[] texturesFiles){
        ...
        GL13.glBindTexture(GL13.GL_TEXTURE_CUBE_MAP, texID);

        for (int i = 0; i < texturesFiles.length; i++) {
            TextureData data = loadTextureData(texturesFiles[i]);
            GL11.glTexImage2D(GL13.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL11.GL_RGBA, data.getWidth(), data.getHeight(), 0, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, data.getBuffer());

            // TODO Peux faire erreru
